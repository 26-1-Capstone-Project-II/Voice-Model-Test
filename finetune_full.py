...
====================================================
기존 finetune_simple.py를 기반으로 하며,
리눅스 서버 환경 경로 및 100% 전용 모델 저장 경로가 세팅되어 있습니다.
...
# 🚨 [필수 추가] 1D-Conv 메모리 폭발 및 cuDNN 충돌 방지막
torch.backends.cudnn.enabled = False

# ────────────────────────────────────────────
# ⚙️ 설정 (서버 100% 학습 환경에 맞게 수정됨)
# ────────────────────────────────────────────
...
# 🚨 [경로 수정] 서버의 실제 데이터 폴더 이름으로 반드시 수정해 주세요!
DEFAULT_WAV_DIR   = "/home/slim/mingly_workspace/원천데이터_폴더명_입력"
DEFAULT_JSON_DIR  = "/home/slim/mingly_workspace/라벨링데이터_폴더명_입력"

# ⭐️ 10% 모델(best_model)과 겹치지 않도록 완전히 새로운 100% 저장 경로 지정
DEFAULT_OUTPUT_DIR= "/home/slim/mingly_workspace/Voice-Model-Test/best_model_100"
...
def train(
    ...
    training_args = TrainingArguments(
        ...
        max_steps                   = max_steps if max_steps > 0 else -1, # -1이면 100% 끝까지 학습
        learning_rate               = lr,
        weight_decay                = 0.005,
        fp16                        = False,  
...
if __name__ == "__main__":
    ...
    # 상단 설정 변수(DEFAULT_...)를 기본값으로 사용합니다.
    ...
    parser.add_argument("--max_steps",  type=int,   default=-1, help="테스트용: -1이면 전체 학습")
