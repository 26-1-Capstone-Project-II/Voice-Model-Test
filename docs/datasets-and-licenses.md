...
작성일: 2026-07-31 (최종 확인: 2026-09-16)
대상: 현재 배포 모델 = 외래어 어휘 확장 런 E
(`best_model_vocab_e/best` → `Whisper_CoreML_Model`, 커밋 `cd4363e5` 학습 / `e1f41b87` 변환 교체)

> 작성 당시 대상은 v3(`best_model_vocab/best`, 커밋 `ba75ec91` — 현재 [../archive/models/](../archive/models/))
> 였으나, 런 E 는 **같은 소스 구성(D1~D7)** 을 파라미터만 바꿔 재학습한 것이라 아래 라이선스 판정은
> 그대로 유효하다. 소스를 추가/교체하면 이 문서를 먼저 갱신할 것.
