...
def load_pairs(wav_dir: Path, json_dir: Path, g2p) -> list[dict]:
    """
    VAD 세그멘테이션 결과(JSONL) 또는 기존 JSON 방식 모두 지원.
    JSONL 파일(train.jsonl 등)이 있으면 그것을 우선 사용.
    """
    # VAD 세그멘테이션 결과 (JSONL) 우선 확인
    jsonl_files = list(json_dir.glob("*.jsonl"))
    if jsonl_files:
        print(f"  📂 JSONL 세그멘테이션 데이터 감지: {[f.name for f in jsonl_files]}")
        pairs = []
...
def train(
    ...
    # 데이터 로딩
    print(f"\n🔗 데이터 매칭 중...")
    pairs = load_pairs(wav_dir, json_dir, g2p)
    if not pairs:
    ...
    val_ds   = DysarthriaDataset(split["validation"], processor)
    ...
    # 테스트 평가
    print("\n📊 Test 평가:")
    test_ds  = DysarthriaDataset(split["test"], processor)
